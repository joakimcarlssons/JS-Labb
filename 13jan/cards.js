...
// Indexes
let index = 0;
let maxIndex = result["results"].length - 1
...
leftArr.addEventListener("click", () => {
    ...
    // If we go back from the first one, we want to show the last one
    if(index < 0) {
        index = maxIndex
    }
...
rightArr.addEventListener("click", () => {
    ...
    // if we are at the last person we go back to beginning
    if (index > maxIndex) {
        index = 0;
    }
...
function fillCards(index) {

    setValues(index, ".first-card")

    if(index == maxIndex){
        setValues(0, ".scnd-card")
        setValues(1, ".third-card")
    }
    else if (index + 1 == maxIndex) {
        setValues(maxIndex, ".scnd-card")
        setValues(0, ".third-card")
    }
    else {
        setValues(index + 1, ".scnd-card")
        setValues(index + 2, ".third-card")
    }
}


// Setting the values
function setValues(index, card) {
        document.querySelector(`${card} .title`).innerHTML = `${result["results"][index]["name"].title}`
        document.querySelector(`${card} .first-name`).innerHTML = `${result["results"][index]["name"].first}`
        document.querySelector(`${card} .last-name`).innerHTML = `${result["results"][index]["name"].last}`
        document.querySelector(`${card} .profile-img`).setAttribute("src", `${result["results"][index]["picture"].large}`)
        document.querySelector(`${card} .email`).innerText = `${result["results"][index].email}`
        document.querySelector(`${card} .phone`).innerText = `${result["results"][index].phone}`
...
// Setting arrows depending on screen size
function setArrows(windowSize){
    document.querySelector(".arrow-left").innerHTML = ""
    document.querySelector(".arrow-right").innerHTML = ""

    if (windowSize.matches) {
        document.querySelector(".arrow-left").innerHTML = "^"
        document.querySelector(".arrow-right").innerHTML = "v"
    }
    else {
        document.querySelector(".arrow-left").innerHTML = "<"
        document.querySelector(".arrow-right").innerHTML = ">"
...
// Run att launch
fillCards(0)
setArrows(windowSize)
windowSize.addListener(setArrows)
